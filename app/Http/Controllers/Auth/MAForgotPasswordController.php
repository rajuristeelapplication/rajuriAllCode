...
class MAForgotPasswordController extends Controller
{
    /*
    |--------------------------------------------------------------------------
    ...
    */
    ...
    {
        return view('ma.auth.forgot_password');
    }
    /**
    ...
    {
        return Password::broker('marketingAdmin');
    }
    ...
    public function checkUserIsAdmin(Request $request)
    {
        ...
        $user = User::where(['email' =>  $request->email,'roleId' => config('constant.ma_id')])->first();
        ...
        if (!empty($user)) {
            ...
            session()->put('passwordBroker', 'marketingAdmin');
