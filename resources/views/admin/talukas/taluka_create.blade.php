...
@section('title') {{ empty($talukaDetail) ? 'Create' : 'Edit' }} {{__('labels.taluka')}} @endsection
@section('css')
...
<div class="content">
    ...
    <div class="-intro-x breadcrumb me-auto d-none d-sm-flex">
        ...
        <a href="{{ route('talukas.index') }}"><span class="breadcrumb">{{ __('labels.talukas') }}</span></a>
        <i class="feather-chevron-right" class="breadcrumb__icon"></i>
        <span class="breadcrumb--active">{{ empty($talukaDetail) ? 'Create' : 'Edit' }} {{ __('labels.taluka')
            }}</span>
    </div>
    @endsection
    ...
    <div class="container-fluid">
        <div class="row page-titles">
            <div class="col-md-5 align-self-center">
                <h4 class="text-themecolor">{{ empty($talukaDetail) ? 'Create' : 'Edit' }} {{
                    __('labels.taluka') }}</h4>
            </div>
        </div>
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="preview">
                            ...
                            @if (empty($talukaDetail))
                            <form class="createTaluka" name="talukaForm" id="talukaForm" method="post"
                                enctype="multipart/form-data" action="{{route('talukas.store')}}">
                                @else
                                <form class="createTaluka" name="talukaForm" id="talukaForm" method="post"
                                    enctype="multipart/form-data" action="{{route('talukas.store')}}">
                                    ...
                                    <input type="hidden" name="id" id="id"
                                        value="{{ !empty($talukaDetail) ? $talukaDetail->id : '' }}" />
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="input-form">
                                                ...
                                                <select id="{{__('labels.state_db')}}" name="{{__('labels.state_db')}}"
                                                    ...
                                                    <option value="{{ $statesInfo->id }}" {{ !empty($talukaDetail->
                                                        stateId) && ($talukaDetail->stateId == $statesInfo->id)
                                        ...
                                        <div class="col-md-6">
                                            <div class="input-form">
                                                <label class="d-flex flex-column flex-sm-row"> {{__('labels.taluka')}}
                                                    <span class="text-danger">*</span> </label>
                                                    <input type="text" name="{{__('labels.taluka_db')}}"
                                                    id="{{__('labels.taluka_db')}}"
                                                    class="form-control input-valid w-100 mt-2 mb-2"
                                                    value="{{  $talukaDetail->tName  ?? '' }}" minlength="3"
                                                    maxlength="20" placeholder="Enter {{__('labels.taluka')}}"
                                                    aria-required="true">
                                            </div>
                                        </div>
    ...
    $(".createTaluka").validate({
        ...
        errorPlacement: function (error, element) {
          console.log(error.text());
             ...
             }else if (error.text() == "Please select city") {
                error.appendTo("#cName-error");
             }
             else{
        ...
        messages: {
            ...
            {{__('labels.taluka_db')}}: {
                required: '{{__('labels.taluka_valid')}}',
            },
        ...
        $("#cName").html('');
            ...
            $.ajax({
                ...
                success: function(result){
                    ...
                    $('#cName').html('<option value="">Select City</option>');
                    $.each(result.city,function(key,value){
                    $("#cName").append('<option value="'+value.id+'">'+value.cName+'</option>');
                    });
                }
            });
        });
    ...
    var ajaxUrl = '{{ route('get-city') }}';
            $.ajax({
                ...
                success: function(result){
                    ...
                    $("#cName").attr("disabled", false);
                    $.each(result.city,function(key,value){
                    $("#cName").append('<option value="'+value.id+'" selected>'+value.cName+'</option>');
                    });
                }
            });
    }
