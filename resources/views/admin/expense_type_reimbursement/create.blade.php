...
@section('title') {{ empty($companyProfileDetail) ? 'Create' : 'Edit' }} {{__('labels.ex_reimbursements')}} @endsection
@section('css')
...
<div class="content">
    ...
    <div class="-intro-x breadcrumb me-auto d-none d-sm-flex">
        ...
        <a href="{{ route('expense-type-reimbursements.index') }}"><span class="breadcrumb">{{ __('labels.ex_reimbursements') }}</span></a>
        <i class="feather-chevron-right" class="breadcrumb__icon"></i>
        <span class="breadcrumb--active">{{ empty($companyProfileDetail) ? 'Create' : 'Edit' }} {{ __('labels.ex_reimbursement')
            }}</span>
    </div>
    @endsection
                    ...
                    <div class="card-body">
                        <div class="preview">
                            ...
                            <form class="createCompanyProfile" name="companyProfileForm" id="companyProfileForm" method="post"
                                enctype="multipart/form-data" action="{{route('expense-type-reimbursements.store')}}">
                                @else
                                <form class="createCompanyProfile" name="companyProfileForm" id="companyProfileForm" method="post"
                                    enctype="multipart/form-data" action="{{route('expense-type-reimbursements.store')}}">
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="input-form">
                                                <label class="d-flex flex-column flex-sm-row"> {{__('labels.ex_reimbursement')}}
                                                    <span class="text-danger">
                                                        *</span> </label>
                                                <input type="text" name="eName" id="eName"
                                                    class="form-control input-valid w-100 mt-2 mb-2"
                                                    value="{{  $companyProfileDetail->eName  ?? '' }}" minlength="4"
                                                    maxlength="255" placeholder="Enter {{__('labels.ex_reimbursement')}}"
                                                    aria-required="true">
                                            </div>
                                        </div>
