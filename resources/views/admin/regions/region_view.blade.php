...
@section('title') {{__('labels.taluka_detail')}} @endsection
@section('content')
...
<div class="content">
    ...
    <div class="-intro-x breadcrumb me-auto d-none d-sm-flex">
        ...
        <a href="{{ route('talukas.index') }}"><span
                class="breadcrumb">{{__('labels.company_profile_flag')}}</span></a>
        <i class="feather-chevron-right" class="breadcrumb__icon"></i>
        <span class="breadcrumb--active">{{__('labels.taluka_detail')}}</span>
    </div>
    @endsection
            ...
            <div class="col-span-12 mt-4">
                <div class="intro-y d-flex align-items-center justify-content-between h-10">
                    <h2 class="text-lg font-medium truncate me-4 mb-0">
                        {{__('labels.taluka_detail')}}
                    </h2>
                    <div class="d-flex ">
                        <a href="{{ route('talukas.edit', ['taluka' => $talukaInfo->id ]) }}"
                            class="ms-auto d-flex btn btn-success"> {{__('labels.edit_info')}} </a>
                        <a href="{{ route('talukas.index') }}" class="ms-3 d-flex btn btn-danger"> {{__('labels.back_info')}} </a>
                    </div>
                </div>
                                                ...
                                                <div
                                                    ...
                                                    <div
                                                        ...
                                                        <div class="col-6 mb-1 mt-1 mr-2 ml-1">
                                                            ...
                                                            <span class=" mr-2 ml-2">{{ $talukaInfo->title ??
                                                                '-' }}</span>
                                                        </div>

                                                        <div class="col-4 mb-1 mt-1 mr-2 ml-1">
                                                            <b>{{__('labels.company_sub_title_list_page')}} :</b>
                                                            <span class=" mr-2 ml-2">{{ $talukaInfo->subTitle ??
                                                                '-' }}</span>
                                                        </div>
                                                    </div>
