...
@section('title') {{__('labels.department_detail')}} @endsection
@section('content')
...
<div class="content">
    ...
    <div class="-intro-x breadcrumb me-auto d-none d-sm-flex">
        ...
        <a href="{{ route('companyProfiles.index') }}"><span class="breadcrumb">{{__('labels.department')}}</span></a>
        <i class="feather-chevron-right" class="breadcrumb__icon"></i>
        <span class="breadcrumb--active">{{__('labels.department_detail')}}</span>
    </div>
    @endsection
            ...
            <div class="col-span-12 mt-4">
                <div class="intro-y d-flex align-items-center justify-content-between h-10">
                    <h2 class="text-lg font-medium truncate me-4 mb-0">
                        {{__('labels.department_detail')}}
                    </h2>
                    <div class="d-flex ">
                        <a href="{{ route('departments.edit', ['department' => $departmentInfo->id ]) }}"
                            class="ms-auto d-flex btn btn-success"> {{__('labels.edit_info')}} </a>
                        <a href="{{ route('departments.index') }}" class="ms-3 d-flex btn btn-danger">
                                                ...
                                                <div
                                                    ...
                                                    <div
                                                        ...
                                                        <div class="col-6 mb-1 mt-1 mr-2 ml-1">
                                                            <b>{{__('labels.department')}} :</b>
                                                            <span class=" mr-2 ml-2">{{ $departmentInfo->dName ??
                                                                '-' }}</span>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div
                                                        ...
                                                        <div class="col-12 mb-1 mt-1 mr-2 ml-1">
                                                            <b>{{__('labels.department_head')}} :</b>


                                                            @foreach ($headDepartmentInfo as $key => $value)
                                                            <br>
                                                            <span class=" mr-2 ml-2"
                                                                style="white-space: normal !important;">{{
                                                                ($key+1).'. '.$value->hdName }}</span>
                                                            @endforeach
                                                        </div>
                                                    </div>
                                                </div>
